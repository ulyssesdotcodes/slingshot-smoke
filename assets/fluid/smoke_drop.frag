...
void main() {
	...
	if(bassDrop > 0.125 && pos.x > 0.2 && pos.x < 0.8 && pos.y > 0.2) {
		float bass = max(0, 0.28 - pos.y) * dt * 256 * bassDrop;
		fragColor += vec4(hsv2rgb(vec3(0, saturation, 1.0)) * bass, 0);
	}
}